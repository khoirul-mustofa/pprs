            ...
            @if ($semuaBerita->count())
                <div class="card mb-3">
                    @if ($semuaBerita[0]->image)
                        <img src="{{ asset('storage/' . $semuaBerita[0]->image) }}" class="center rounded "
                            alt="...">
                    @else
                    ...
                    <div class="card-body text-center">
                        ...
                        <small class="fw-normal font-Roboto">
                            Oleh: <a class="text-decoration-none"
                                href="/berita?author={{ $semuaBerita[0]->user->username }}">{{ $semuaBerita[0]->user->name }}</a>
                            pada kategori <a class="text-decoration-none"
                                href="/berita?kategori={{ $semuaBerita[0]->kategori->name }}">{{ $semuaBerita[0]->kategori->name }}</a>
                    ...
                    @foreach ($semuaBerita->skip(1) as $post)
                        <div class="col">
                            <div class="card shadow-lg">
                                <div class="py-1 px-1 py-md-2 px-md-3 position-absolute rounded"
                                    style="background-color: rgba(0, 0, 0, 0.7)">
                                    <a href="/berita?kategori={{ $post->kategori->name }}"
                                        class="text-decoration-none text-light"> {{ $post->kategori->name }}</a>
                                </div>
                                @if ($post->image)
                                    <img src="{{ asset('storage/' . $post->image) }}" class="center rounded"
                                        alt="...">
                                @else
                                ...
                                <div class="card-body">
                                    ...
                                    <h5 class="card-text fw-semibold text-decoration-none text-dark">{{ $post->title }}
                                    </h5>

                                    <small class="fw-normal font-Roboto">Oleh: <a class="text-decoration-none"
                                            href="/berita?author={{ $post->user->username }}">{{ $post->user->name }}</a>
                                        <small
                                            class="text-muted">{{ $post->created_at->diffForHumans() }}</small></small>
